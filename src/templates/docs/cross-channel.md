...
- A tool behaved unexpectedly (seal, maw, bus, vessel, bn) — **ask the responsible project**
...
```bash
bus send --agent $AGENT <project> "Getting error X when running seal inbox. Is this expected? Here's what I see: <details> @<project>-dev" -L feedback
```
