...
**Important:** {% if is_root_layout %}Run `bn` commands directly (e.g., `bn do ...`) — they operate on the trunk at the repo root.{% else %}Run all `bn` commands via `maw exec default --` (e.g., `maw exec default -- bn do ...`). This ensures they always run in the default workspace context.{% endif %} Run `seal` commands via `maw exec $WS --` to target the correct workspace.
...
- **All file operations must use the workspace path** `{{ ws_prefix }}$WS/`. Use absolute paths for Read, Write, and Edit (e.g., `$PROJECT_ROOT/{{ ws_prefix }}$WS/src/file.rs`). For commands: `maw exec $WS -- <command>`.
...
- Add a comment to the bone: `{{ bn }} bone comment add <bone-id> "Review requested: <review-id>, workspace: $WS ({{ ws_prefix }}$WS/)"`
...
- Add comment to bone: `{{ bn }} bone comment add <bone-id> "Review requested: <review-id>, workspace: $WS ({{ ws_prefix }}$WS/)"`
